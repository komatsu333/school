...
def get_latest_frame_safe(sock):
        ...
        try:
                ...
                # デコード試行
                frame = tcp_client.decode_camera(frame_data)
                if frame is None:
                        consecutive_errors += 1
                        if consecutive_errors >= 3:
...
def init_gui():
        ...
        while sock is None:
                try:
                        print(f"TCP接続を試みています...")
                        sock = tcp_client.connect_server(tcp_client.IPADDR, [tcp_client.PORT1, tcp_client.PORT2])
                        if sock is not None:
                                print("TCP接続が確立しました")
                                break
                        else:
        ...
        #speed_btn
        speed_btn = tk.Button(root, text=f"現在：{status}", width=6, height=3, highlightthickness=0, command=lambda: (send_xbee.send("CH\n"), ch_speed(speed_btn)), repeatdelay=btn_delay, repeatinterval=btn_interval, background="white", activebackground="white")
        speed_btn.place(x=x2-5, y=speed_y)

        #direction_btn
        f_btn  = tk.Button(root, text="↑", width=btn_w, height=btn_h, command=lambda: send_xbee.send("F\n"),
        ...
        fr_btn = tk.Button(root, text="↗", width=btn_w, height=btn_h, command=lambda: send_xbee.send("FR\n"),
                            repeatdelay=btn_delay, repeatinterval=btn_interval, background="white", activebackground="white")
        fr_btn.place(x=x3, y=y1)

        r_btn  = tk.Button(root, text="→", width=btn_w, height=btn_h, command=lambda: send_xbee.send("R\n"),
                            repeatdelay=btn_delay, repeatinterval=btn_interval, background="white", activebackground="white")
        r_btn.place(x=x3, y=y2)

        br_btn = tk.Button(root, text="↘", width=btn_w, height=btn_h, command=lambda: send_xbee.send("BR\n"),
                            repeatdelay=btn_delay, repeatinterval=btn_interval, background="white", activebackground="white")
        br_btn.place(x=x3, y=y3)

        b_btn  = tk.Button(root, text="↓", width=btn_w, height=btn_h, command=lambda: send_xbee.send("B\n"),
                            repeatdelay=btn_delay, repeatinterval=btn_interval, background="white", activebackground="white")
        b_btn.place(x=x2, y=y3)

        bl_btn = tk.Button(root, text="↙", width=btn_w, height=btn_h, command=lambda: send_xbee.send("BL\n"),
                            repeatdelay=btn_delay, repeatinterval=btn_interval, background="white", activebackground="white")
        bl_btn.place(x=x1, y=y3)

        l_btn  = tk.Button(root, text="←", width=btn_w, height=btn_h, command=lambda: send_xbee.send("L\n"),
                            repeatdelay=btn_delay, repeatinterval=btn_interval, background="white", activebackground="white")
        l_btn.place(x=x1, y=y2)

        fl_btn = tk.Button(root, text="↖", width=btn_w, height=btn_h, command=lambda: send_xbee.send("FL\n"),
                            repeatdelay=btn_delay, repeatinterval=btn_interval, background="white", activebackground="white")
        fl_btn.place(x=x1, y=y1)

        #stop_btn - stop
        st_btn = tk.Button(root, text="STOP", width=btn_w, height=btn_h, command=lambda: send_xbee.send("ST\n"),
                            repeatdelay=btn_delay, repeatinterval=btn_interval, background="white", activebackground="white")
        ...
        #servo_btn
        up_btn = tk.Button(root, text="△", width=5, height=2, command=lambda: send_xbee.send("UP\n"),
                            repeatdelay=btn_delay, repeatinterval=btn_interval, background="white", activebackground="white")
        up_btn.place(x=btn_x, y=y2)

        down_btn = tk.Button(root, text="▽", width=5, height=2, command=lambda: send_xbee.send("DO\n"),
                            repeatdelay=btn_delay, repeatinterval=btn_interval, background="white", activebackground="white")
        down_btn.place(x=btn_x, y=y3)

        #exit_btn
        #exit_btn = tk.Button(root, text="exit", width=5, height=3, command=lambda: (send_xbee.send("EX\n"), os.system("/usr/bin/sudo /sbin/poweroff")),
        #                    repeatdelay=btn_delay, repeatinterval=btn_interval, background="white", activebackground="white")
